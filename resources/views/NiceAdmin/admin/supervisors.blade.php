        ...
        <div class="modal-content">
            ...
            <form action="{{route('add-supervisor')}}" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="alert alert-danger">You cannot register supervisors, <a href="/admin/programmes">Add programmes first</a></div>
                    @else
...
<!-- END A MODAL TO UPLOAD A supervisor -->
    ...
    <div class="card recent-sales overflow-auto">
        <div class="card-body">
            <div class="col-12 d-flex justify-content-between align-items-center">
                ...
                <div>
                    <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#supervisorModal">Add supervisor</button>
                </div>
            </div>
            <table class="table table-borderless datatable">
                ...
                <tbody>
                    ...
                    <tr>
                        <td colspan="5" class="bg-light">No available supervisor</td>
                    </tr>
                    @endif
                    @foreach($supervisors as $supervisor)
                    <tr>
                        <th scope="row">{{$loop->iteration}}</th>
                        <td>{{$supervisor->users->firstname}}&nbsp;{{$supervisor->users->middlename}}&nbsp;{{$supervisor->users->lastname}}</td>
                        <td>{{$supervisor->users->email}}</td>
                        <td>{{$supervisor->programmes->program_code}}</td>
                        <td>{{ \Carbon\Carbon::parse($supervisor->created_at)->diffForHumans() }}</td>
