                    ...
                    <div class="modal fade" id="requestModal{{$request['id']}}" tabindex="-1" aria-labelledby="requestModalLabel" aria-hidden="true">
                        <div class="modal-dialog modal-lg">
                            <div class="modal-content">
                                ...
                                <form action="{{route('dos_remark_request')}}" method="POST">
                                    @csrf
                                    <div class="modal-body">
                                        <input class="form-control" name="dof_remark_id" value="{{$request['id']}}" hidden />
                                        ...
                                        <div class="form-group row my-3">
                                            <div class="col-md-4">
                                                <div class="form-check form-check-inline">
                                                    <input type="radio" class="form-check-input" id="accept-radio" name="response" value="1" {{old('response') == 'accepted' ? 'checked' : ''}}>
                                                    <label class="form-check-label text-success fw-bolder" for="accept-radio">Accepted</label>
                                                </div>
                                            </div>
                                            <div class="col-md-4">
                                                <div class="form-check form-check-inline">
                                                    <input type="radio" class="form-check-input" id="reject-radio" name="response" value="0" {{old('response') == 'rejected' ? 'checked' : ''}}>
                                                    <label class="form-check-label text-danger fw-bolder" for="reject-radio">Rejected</label>
                                                </div>
                                            </div>
                                            @error('response')
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <button type="submit" class="btn btn-primary"><i class="fa fa-save"></i>Recommend</button>
                                        </div>
                                    </div>
                                </form>
                            </div>
